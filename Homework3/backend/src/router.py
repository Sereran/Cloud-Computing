...
async def get_aggregated_game_data(game_id: int):
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            rawg_resp, cs_resp, rates_resp = await asyncio.gather(rawg_task, cs_task, rates_task)
        except httpx.RequestError as e:
            logger.error(f"Failed to connect to external APIs: {e}")
            raise HTTPException(status_code=502, detail="Failed to connect to external APIs.")
        ...
        if rawg_resp.status_code == 200 and rawg_data.get("results"):
            ...
            cover_image, metacritic = item.get("background_image"), item.get("metacritic")
            platforms = [p.get("platform", {}).get("name") for p in item.get("platforms", []) if p.get("platform", {}).get("name")]

        cs_data = cs_resp.json()
        price_usd, deal_id = (float(cs_data[0]["cheapest"]), cs_data[0]["cheapestDealID"]) if cs_resp.status_code == 200 and cs_data else (None, None)

        price_eur, price_ron = None, None
        if rates_resp.status_code == 200 and price_usd is not None:
            rates = rates_resp.json().get("rates", {})
            price_eur, price_ron = round(price_usd * rates.get("EUR", 1.0), 2), round(price_usd * rates.get("RON", 1.0), 2)

        logger.info(f"Successfully aggregated data for game '{game_title}' ({game_id}).")
        return {
            "local_data": game_data,
            "local_media": local_media_urls,
            "media": {"cover_url": cover_image},
            "pricing": {"usd": price_usd, "eur": price_eur, "ron": price_ron, "deal_id": deal_id},
...
async def create_game(request: Request):
    ...
    title, desc, tags = data.get("title"), data.get("description", ""), data.get("tags", [])
    
    with db.db_session() as conn:
        conn.start_transaction()
        with conn.cursor() as cursor:
            try:
                cursor.execute("INSERT INTO game (title, description) VALUES (%s, %s)", (title, desc))
                new_id = cursor.lastrowid
                
                for tag_name in tags:
                    cursor.execute("INSERT IGNORE INTO tag (name) VALUES (%s)", (tag_name,))
                    cursor.execute("INSERT INTO applied_tag (game_id, tag_name) VALUES (%s, %s)", (new_id, tag_name))
        ...
        with conn.cursor() as cursor:
            try:
                for table in ["applied_tag", "library", "media", "game"]:
                    cursor.execute(f"DELETE FROM {table} WHERE game_id = %s" if table != "game" else f"DELETE FROM {table} WHERE id = %s", (game_id,))
